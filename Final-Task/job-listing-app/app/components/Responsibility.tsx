...
const Responsibility = ({responsibilities}:JobType) => {
    const responsibilitiesLine = responsibilities.split("\n");
  return (
     <div className='mb-[16px]'>
        <p className='font-[900] text-[24px] leading-[1.2] mb-4'>Responsibilities</p>
        <div>
           <ul className='space-y-3'>
          {responsibilitiesLine.map((item, index) => (
            <li key={index} className="flex items-start gap-2">
              ...
              <span className="text-[17px] leading-[1.6]">{item}</span>
            </li>
          ))}
        </ul>
        </div>
